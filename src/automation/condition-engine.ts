...
export class ConditionEngine {
  ...
  public generateConditionEffects(ability: ParsedAbility): {
    ...
    // Generate Specter Summoning effect (Wraith)
    if (conditionAnalysis.specterSummoning) {
      const specterEffect = this.generateSpecterSummoningEffect(ability);
      effects.push(specterEffect);
      macros.push(this.generateSpecterSummoningMacro(ability));
      flags['specterSummoning'] = true;
    }
  ...
  private analyzeConditionText(text: string): ConditionAnalysis {
    ...
    // Specter Summoning detection (Wraith)
    if (text.match(/dies within 1 minute.*rises as a specter/i)) {
      analysis.specterSummoning = true;
    }
...
// Allow "isDamaged" (MidiQOL), "on" (initial), and "each" (DAE turn loops)
if (macroPass !== "isDamaged" && macroPass !== "on" && macroPass !== "each") return;

// Robust actor retrieval for both MidiQOL and DAE contexts
const actor = args[0]?.actor ?? (typeof actor !== "undefined" ? actor : canvas.tokens.controlled[0]?.actor);
if (!actor) return;
...
`;
  ...
  private generateSpecterSummoningEffect(ability: ParsedAbility): ActiveEffectData {
    return {
      _id: generateFoundryId(),
      name: `${ability.name} - Marked for Undeath`,
      img: "icons/magic/death/skull-horned-goat-purple.webp",
      changes: [
        {
          key: "flags.midi-qol.onUseMacroName",
          mode: 0, // CUSTOM
          value: "ItemMacro, zeroHP",
          priority: 20
        }
      ],
      duration: {
      ...
      flags: {
        ...
        automancy: {
          generated: true,
          specterSummoning: true
        }
      },
      statuses: ["marked-for-undeath"],
      transfer: false,
      disabled: false
    };
  }

  private generateSpecterSummoningMacro(ability: ParsedAbility): string {
    return `
// Specter Summoning Automation for ${ability.name}
// Triggered on ZeroHP via MidiQOL

if (args[0].macroPass !== "zeroHP" && args[0].tag !== "zeroHP") return;

const targetToken = args[0].token || canvas.tokens.get(args[0].tokenId);
const targetActor = args[0].actor || targetToken?.actor;

if (!targetActor || !targetToken) return;

// Check if target is Humanoid
const race = targetActor.system.details.race?.toLowerCase() || "";
const type = targetActor.system.details.type?.value?.toLowerCase() || "";

if (!race.includes("humanoid") && !type.includes("humanoid")) {
  console.log("Target is not humanoid, no specter rises.");
  return;
}

try {
  // Spawn Specter at target location
  const specterData = {
    name: "Specter",
    type: "npc",
    img: "systems/dnd5e/icons/svg/items/spectral.svg", // Placeholder
    system: {
      attributes: { hp: { value: 22, max: 22 } } // Basic Specter HP
    }
  };
  
  // Use Warpgate if available, otherwise just chat message
  if (game.modules.get("warpgate")?.active) {
    await warpgate.spawn("Specter", {
      token: { x: targetToken.x, y: targetToken.y }
    });
    
    ChatMessage.create({
      content: \`<strong>\${targetToken.name}'s</strong> spirit rises as a <strong>Specter</strong>!\`
    });
  } else {
    ui.notifications.warn("Specter summoning requires Warpgate module for auto-spawn.");
    ChatMessage.create({
      content: \`<strong>\${targetToken.name}</strong> dies and their spirit rises as a <strong>Specter</strong>! (GM: Please spawn a Specter here)\`
    });
  }

} catch (err) {
  console.error("Specter summoning failed:", err);
}
`;
  }
